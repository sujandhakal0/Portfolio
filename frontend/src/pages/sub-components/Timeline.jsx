...
const Timeline = () => {
  ...
  useEffect(() => {
    const getTimeline = async () => {
      const { data } = await axios.get(
        "https://portfolio-backend-91np.onrender.com/api/v1/timeline/getall",
  ...
  return (
    <>
      ...
      <ol className="relative border-s border-gray-500 dark:border-gray-700 ml-6">
        {timeline &&
            ...
            return (
              <li className="mb-10 ms-6" key={element._id}>
                <span className="absolute flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full -start-3 ring-8 ring-white dark:ring-gray-900 dark:bg-blue-900">
                  <svg
                    className="w-2.5 h-2.5 dark:text-green-500 text-green-800 "
                    aria-hidden="true"
                ...
                <h3 className="mb-1 text-lg font-semibold dark:text-gray-200 text-gray-900">
                  {element.title}
                </h3>
